show_save_statistics <- function(cov_names, res, RND_samples, dsn_folder){
  ...
  for(i in 1:length(cov_names)){
    ...
    if(!is.numeric(tmp_cov))  
      tmp_cov <- as.numeric(tmp_cov)
    hist(tmp_cov, main = paste("RND histogram of",cov_names[i]), xlim = x_lim, xlab = paste("Mean:",mean(tmp_cov, na.rm = TRUE), ", Variance", var(tmp_cov, na.rm = TRUE)), freq = FALSE)
